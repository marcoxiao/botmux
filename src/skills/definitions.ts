...
const SEND_SKILL = `---
name: botmux-send
description: 显式向飞书话题发送消息，用于当前 routing 允许的中途推送、附件、卡片、@mention、跨群/顶层发布或 --attention；普通最终答复是否使用本命令，以当前 botmux_routing / botmux_reminder 为准。支持图文混排、文本及图片/文件附件。**当你自主执行任务撞到只有人类才能解除的硬阻碍、无法靠自己继续时（需要授权/凭证、要人拍不可逆决策、缺访问权限、需求歧义自己定不了），回消息时带 \`--attention\` 举手**——既把"我卡在哪、需要你做什么"发给用户，又把本会话标进 dashboard「需要你」列，让人一眼看到哪个任务卡住、为什么卡。
---

# botmux-send — 向飞书话题发送消息

**交付优先级**：当前注入的 \`botmux_routing\` / \`botmux_reminder\` 优先。若其中要求普通答复写入 \`final\`，就只把中途主动推送、附件/卡片或 @ 接力交给 \`botmux send\`，普通最终答复留在 \`final\`；若没有这项约定，用户在飞书看不到终端输出，需要用户看到的内容才通过本命令发送。
...
- 关键结论、方案（等用户确认再执行）
- 最终结果（仅当当前 routing 未指定普通答复写入 \`final\`）
- 进度更新（长任务的中途汇报）
- 需要用户回复的问题
